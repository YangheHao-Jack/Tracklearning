...
class ShapeSensitiveLoss(nn.Module):
    """
    Shape-Sensitive Loss based on cosine similarity between features extracted
    from the Signed Distance Maps (SDMs) of predicted and true masks.
    """
    def __init__(self, smooth=1e-6):
    ...
    def forward(self, pred_logits, true):
        ...
        # Resize SDMs to the ViT expected resolution (384x384)
...
# ------------------------------
# 3. Compute Signed Distance Map (SDM)
# ------------------------------
...
# ------------------------------
# 4. ViT Feature Extractor for Shape-Sensitive Loss
# ------------------------------
...
# ------------------------------
# 6. Combined Loss: Dice Loss + Shape-Sensitive Loss
# ------------------------------
...
# ------------------------------
# 7. SegViT Model Definition (ResNet Encoder + Transformer Bridge + U-Net Decoder)
# ------------------------------
...
# ------------------------------
# 11. Training and Evaluation Functions (with Mixed Precision)
# ------------------------------
...
def main():
    ...
    learning_rate = 1e-4
    val_split     = 0.2
    save_path     = "best_segvit_model10.pth"
    patience      = 10
    ...
    print("[Main] Calculating class weights... (Not used for shape-sensitive loss)")
    print("[Main] Class weights not used for shape-sensitive loss.")

    # Here, we use SegViT (a ResNet encoder + Transformer Bridge + U-Net style decoder).
    model = SegViT(encoder_name='resnet50', pretrained=True, num_classes=2).to(device)
    # Combined loss: Dice loss + Shape-Sensitive loss.
    ...
    model.eval()
    test_metrics_results = test_segmentation(model, test_loader, device, metrics_test)
