...
class CombinedClDiceDiceLoss(nn.Module):
    """
    Combined clDice and Dice Loss with weights 0.9 and 0.1 respectively.
    """
    def __init__(self, dice_weight=0.1, cldice_weight=0.9, smooth=1e-6):
        super(CombinedClDiceDiceLoss, self).__init__()
    ...
    def forward(self, logits, true):
        ...
        combined_loss = self.cldice_weight * cldice_loss + self.dice_weight * dice_loss
    ...
    def __init__(self, size=(512, 512), augment=False):
        ...
        if self.augment:
            self.transform = A.Compose([
                ...
                A.Transpose(p=0.5),
                A.ElasticTransform(alpha=1, sigma=50, p=0.2),  # Ensure your Albumentations version supports this
                A.GridDistortion(p=0.2),
                A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.2, p=0.3),
                A.GaussNoise(p=0.3),  # Ensure your Albumentations version supports this
...
class TemporalSmoothing:
    ...
    def smooth(self, pred):
        ...
        if len(self.queue) == 0:
            return pred
        # Compute the mean across the queue
...
def visualize_predictions(model, dataset, device, num_samples=3, post_process_flag=False):
    ...
    rng = np.random.default_rng()  # Creates a new generator with a random seed
    ...
    for idx in indices:
        ...
        image_np = image_np * 0.485 + 0.229  # Unnormalize (assuming ImageNet normalization)
        ...
        print(f"Image Shape: {image_np.shape}")       # Expected: [H, W, 3]
        print(f"Mask Shape: {mask_np.shape}")         # Expected: [H, W]")
        
        # Create a copy for overlay to keep the original image unchanged
        overlay_image = image_np.copy()

        # Create a binary mask
        binary_mask = (pred == 1).astype(np.uint8)  # Assuming class 1 is catheter

        # Ensure binary_mask is 2D
        if binary_mask.ndim == 3 and binary_mask.shape[2] == 1:
            binary_mask = binary_mask.squeeze(2)
        
        # Define color for overlay (e.g., green)
        ...
        # Overlay the mask on the image copy
...
def main():
    ...
    # Initialize Combined Loss (clDice + Dice)
    criterion = CombinedClDiceDiceLoss(
        dice_weight=0.1,    # Weight for Dice Loss
        cldice_weight=0.9,  # Weight for clDice Loss
        smooth=1e-6
    )
    ...
    # ------------------------------
    # Optional: Save Training and Validation Loss Curves
    # ------------------------------
