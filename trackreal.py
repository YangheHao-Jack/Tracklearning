...
def initialize_video_io(input_path, output_path, fps=30.0, target_size=(512, 512), rotate_clockwise90=False, use_camera=False):
    """
    Initializes video capture and writer objects, adjusting frame size based on resizing and rotation.

    Args:
        input_path (str): Path to the input video file. Ignored if use_camera is True.
        ...
        use_camera (bool): Whether to use the live camera as input.
    ...
    # Get original frame size from input video (only relevant if not using camera)
    if not use_camera:
        original_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        original_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    else:
        # For camera, you might want to set a specific resolution
        # This depends on your camera's capabilities
        # Here, we'll attempt to set it, but it's optional
...
def process_video(model, cap, out, device='cpu', target_size=(512, 512), rotate_clockwise90=False, post_process_flag=False, crf_flag=False, smooth_flag=False, display=True):
    """
    Processes each frame of the input video or live camera, performs segmentation, overlays the mask, displays the results, and writes to output.
    ...
    Args:
        ...
        display (bool): Whether to display the frames in real-time.
    """
    # Determine total frames (if possible)
    if not cap.isOpened() or cap.get(cv2.CAP_PROP_FRAME_COUNT) <= 0:
        frame_count = float('inf')  # Live camera doesn't have a predefined frame count
    else:
        frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    print(f"Total frames to process: {'Infinite (Live Camera)' if frame_count == float('inf') else frame_count}")
        ...
        while True:
            ...
            if not ret:
                if frame_count == float('inf'):
                    # Live camera: continue to wait for frames
            ...
            input_tensor, processed_frame = preprocess_frame(
                frame, 
                device=device, 
            ...
            overlayed_frame = overlay_mask_on_frame(
                processed_frame, 
                pred_original, 
                color=(0, 255, 0), 
                alpha=0.5
            ...
            # Display the results in separate windows if enabled
            if display:
...
def main():
    """
    Main function to perform real-time segmentation on a live camera or input video using the trained SegViT model.
    """
    # ------------------------------
    ...
    # ------------------------------
    # Select Input Source: Video File or Live Camera
    # ------------------------------
    use_camera = False # Set to True to use the live camera, False to use a video file

    if use_camera:
        print("[Main] Using live camera as input.")
    else:
        print(f"[Main] Using video file as input: {input_video_path}")
    ...
    print(f"Output frame size: {frame_size}")

    # ------------------------------
    # Process the Video or Live Camera
    # ------------------------------
    ...
    process_video(
        ...
        display=True  # Set to False if you do not wish to display the output in real-time
    )
